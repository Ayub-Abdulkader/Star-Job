<x-layout>
    <x-navbar/>
     <div class="container flex flex-col items-center pt-5 bg-no-repeat bg-contain h-screen" style="background-image: url({{url('/images/bg_star.png')}})">
        <div class="bg-white border rounded-lg w-1/2 shadow-md md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">    
            <form class="p-8" method="POST" action="{{route('jobs.store')}}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid md:grid-cols-2 md:gap-6">
                    ...
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="text" name="tags" id="tags" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer placeholder:italic placeholder:text-slate-400 " value="{{old('tags')}}" placeholder="" required />
                ...
                <div class="relative z-0 w-full mb-6 group">
                    ...
                    <input name="logo" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" value="{{old('logo')}}" id="logo" type="file">
                    @error('logo')
                        <p class="text-red-400 mt-1 text-sm">{{$message}}</p>
                    @enderror
                </div>
                <div class="relative z-0 w-full mb-6 group">
                    ...
                    <textarea id="description" name="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="job description here...">{{old('description')}}</textarea>
                    @error('description')
                        <p class="text-red-400 mt-1 text-sm">{{$message}}</p>
                    @enderror
                </div>
