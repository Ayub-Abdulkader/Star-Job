<x-layout>
    <x-navbar/>
     <div class="container flex flex-col items-center pt-10 bg-no-repeat bg-contain h-screen" style="background-image: url({{url('/images/bg_star.png')}})">
        <h1 class="mb-4 text-xl font-bold pb-4 leading-none tracking-tight text-gray-900 md:text-2xl lg:text-6xl dark:text-white"><span class="text-blue-600 dark:text-blue-500">Login </span></h1>
        <div class="bg-white border w-1/2 rounded-lg shadow-md md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">        
            <form class="p-8" method="POST" action="{{route('user.login')}}">
                @csrf
                ...
                <p class="font-sm text-gray-600 dark:text-blue-500 text-center pt-5">Don't have an account 
                    <a href="{{route('user.register')}}" class="font-sm text-blue-600 dark:text-blue-500 hover:underline">Register here</a>
                </p>
            </form>  
